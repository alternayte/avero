...
import (
	...
	"github.com/alternayte/avero/cli/verify"
...
func TestTheSpaBinaryCarriesItsFrontEnd(t *testing.T) {
	...
	// The description is a generated file, so the check proves it. A client
	// of a front end that follows a stale openapi.json states a route that
	// the service does not hold.
	if code, _, errOut := run(t, app, "generate", "--check"); code != 0 {
		t.Fatalf("avero generate --check returned %d after the build: %s", code, errOut)
	}
	if err := os.WriteFile(filepath.Join(app, "openapi.json"), []byte("{}\n"), 0o644); err != nil {
		t.Fatalf("WriteFile returned %v", err)
	}
	code, _, errOut = run(t, app, "generate", "--check")
	if code != 1 || !strings.Contains(errOut, "openapi.json") {
		t.Fatalf("the check accepted a stale description: %d %s", code, errOut)
	}
	if code, _, errOut := run(t, app, "generate"); code != 0 {
		t.Fatalf("avero generate returned %d: %s", code, errOut)
	}
	if code, _, errOut := run(t, app, "generate", "--check"); code != 0 {
		t.Fatalf("avero generate --check returned %d after generate: %s", code, errOut)
	}
