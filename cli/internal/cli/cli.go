...
func init() {
	commands = []command{
		{Name: "new", Usage: "avero new <name> [--shape ssr|spa|api] [--hypermedia datastar|htmx] [--package-manager npm|bun|pnpm|yarn] [--module <path>] [--replace <dir>]",
...
//
// The binary reads its own build, so `go install
// github.com/alternayte/avero/cli/cmd/avero@v0.5.0` states v0.5.0 and a build
// of a working copy states dev. No release step writes the value, so the
// version can never disagree with the binary.
func Version() string {
