...
import (
	...
	"github.com/alternayte/avero/cli/internal/cli"
)
...
// drel writes the columns and the migrations of the new feature, so the
// application must carry the tool. A directory without it states the one
// command that adds it.
//
// The command writes no file when the tool is absent. A slice states its own
// migrations, so a half-written slice does not compile. See DX-8.
func TestSliceNeedsTheDrelTool(t *testing.T) {
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "go.mod"), []byte("module blog\n\ngo 1.26.2\n"), 0o644); err != nil {
		t.Fatalf("WriteFile returned %v", err)
	}
	code, _, errOut := run(t, dir, "slice", "comment")
	if code != 1 || !strings.Contains(errOut, "go get -tool") {
		t.Fatalf("the code is %d and the fault is %q", code, errOut)
	}
	proveNoSlice(t, dir)
}
...
// The description comes from the running application now, so a directory that
// holds no application states the repair. A scaffolded application proves the
// document itself. See TestTheOpenAPIDescriptionOfAScaffoldedApplication.
func TestRoutesNeedsAnApplicationForTheDescription(t *testing.T) {
	code, _, errOut := run(t, t.TempDir(), "routes", "--openapi")
	if code != 1 {
		t.Fatalf("the code is %d, want 1", code)
	}
	if !strings.Contains(errOut, "avero new") {
