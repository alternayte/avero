...
func runNew(ctx context.Context, s Streams, args []string) int {
	...
	for i := 0; i < len(args); i++ {
		...
		case a == "--package-manager" || a == "-package-manager":
			i++
			if i >= len(args) {
				return failf(s, "avero new: --package-manager names no tool\n  → Write --package-manager npm, bun, pnpm or yarn")
			}
			opts.PackageManager = args[i]
		case strings.HasPrefix(a, "--package-manager="):
			opts.PackageManager = strings.TrimPrefix(a, "--package-manager=")
...
func Vendor(ctx context.Context, dir, shape string, s Streams) error {
	for _, module := range FrontEnd[shape] {
		if err := pipeline.PinJS(ctx, pipeline.PinConfig{Dir: dir}, module.Name, module.URL); err != nil {
			return err
		}
