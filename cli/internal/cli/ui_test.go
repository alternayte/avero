...
func fakeBasecoat(t *testing.T) pipeline.Fetcher {
	t.Helper()
	url := cli.BasecoatURL(pipeline.BasecoatVersion)
...
func uiProject(t *testing.T) (string, pipeline.Fetcher) {
	t.Helper()
	dir := t.TempDir()
...
func runUI(t *testing.T, dir string, fetch pipeline.Fetcher, args ...string) (int, string, string) {
	t.Helper()
