...
import (
	...
	"github.com/alternayte/avero/cli/dev"
...
func runDev(ctx context.Context, s Streams, args []string) int {
	...
	if project.Assets.Tier == "external" {
		...
		tool, toolErr := NodeToolOf(project, frontEnd(dir, project))
		if toolErr != nil {
			return fail(s, toolErr)
		}
		front = tool.Script(ScriptDev)
		if _, cmdErr := tool.Command(ctx, front); cmdErr != nil {
			return fail(s, cmdErr)
		}
	}
	...
	server, err := dev.New(dev.Config{
		...
		AssetDir:    filepath.Join(dir, filepath.FromSlash(pipeline.OutDir)),
		BuildCSS: func(ctx context.Context) (string, error) {
			m, err := pipeline.Build(ctx, project.assetConfig(dir, false))
			if err != nil {
				return "", err
			}
		...
		BuildScript: func(ctx context.Context) error {
			...
			_, err := pipeline.Build(ctx, project.assetConfig(dir, false))
			return err
		},
