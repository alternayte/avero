...
import (
	...
	"github.com/alternayte/avero/cli/internal/cli/uifiles"
...
func runUI(ctx context.Context, s Streams, args []string) int {
	...
	cfg := pipeline.PinConfig{Dir: dir, Fetch: s.Fetch}
	if err := pipeline.PinPackage(ctx, cfg, "basecoat", BasecoatURL(pipeline.BasecoatVersion)); err != nil {
		return fail(s, err)
	}
	_, _ = fmt.Fprintf(s.Out, "pinned basecoat %s\n", pipeline.BasecoatVersion)
