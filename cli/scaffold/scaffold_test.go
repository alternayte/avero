...
// An application that adopts Avero holds no drel.yaml. The first slice writes
// the file with its own entry, because drel refuses an empty modules block.
func TestWriteSliceWritesTheDrelConfigurationWhenItIsAbsent(t *testing.T) {
	t.Setenv("DATABASE_URL", "")
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "go.mod"), []byte("module example.test/app\n\ngo 1.26.2\n"), 0o644); err != nil {
		t.Fatalf("WriteFile returned %v", err)
	}
	if _, err := scaffold.WriteSlice(scaffold.SliceOptions{Name: "maker", Dir: dir}); err != nil {
		t.Fatalf("WriteSlice returned %v", err)
	}
	body, err := os.ReadFile(filepath.Join(dir, "drel.yaml"))
	if err != nil {
		t.Fatalf("the slice wrote no drel.yaml: %v", err)
	}
	source := string(body)
	for _, want := range []string{
		"modules:\n  - name: makers\n",
		"./internal/features/makers/model",
		"dialect: sqlite",
	} {
		if !strings.Contains(source, want) {
			t.Fatalf("drel.yaml holds no %q:\n%s", want, source)
		}
	}
}

// The dialect follows DATABASE_URL, because the migrations that drel writes
// carry the types of one server.
func TestTheFirstDrelConfigurationReadsTheDatabaseURL(t *testing.T) {
	t.Setenv("DATABASE_URL", "")
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "go.mod"), []byte("module example.test/app\n\ngo 1.26.2\n"), 0o644); err != nil {
		t.Fatalf("WriteFile returned %v", err)
	...
	if !strings.Contains(string(body), "dialect: postgres") {
		t.Fatalf("drel.yaml names another dialect:\n%s", body)
	}
}

// The CLI is a module of its own, so the version of the host stands in the
// dependencies of the build. A new application requires that version, and not
// the version of the CLI.
func TestAveroVersionNamesTheHostAndNotTheCLI(t *testing.T) {
	version := scaffold.AveroVersion()
	if version == "" {
		// A test binary carries no release version. The scaffolder then
		// writes no require line, which go mod tidy fills.
		return
	}
	if !strings.HasPrefix(version, "v") {
		t.Fatalf("the version %q names no release", version)
	}
	info, ok := debug.ReadBuildInfo()
	if !ok {
		t.Skip("the build carries no information")
	}
	for _, dep := range info.Deps {
		if dep != nil && dep.Path == scaffold.HostModule {
			return
		}
	}
	t.Fatalf("the build names no %s, and the version is %q", scaffold.HostModule, version)
}
