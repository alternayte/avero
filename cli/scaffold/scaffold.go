...
// The package managers that the front end of the spa shape can use. The names
// match the members of internal/cli, which drives them.
const (
...
type Options struct {
	...
	GoVersion string
	// PackageManager is the tool that reads the dependencies and runs the
	// scripts of the front end of the spa shape: npm, bun, pnpm or yarn. An
	// empty value writes npm.
...
type data struct {
	...
	GoVersion  string
	Secret     string
	// PackageManager is the tool of the front end. avero.json names it, and
	// `avero build` and `avero dev` run it.
	PackageManager string
}
...
func validate(opts *Options) error {
	...
	default:
		return faultOf(fmt.Sprintf("the package manager %q is not known", opts.PackageManager),
			"Write --package-manager npm, bun, pnpm or yarn")
	}
	if opts.Module == "" {
...
// HostModule is the module path of the host, which a new application
// requires.
const HostModule = "github.com/alternayte/avero"

// AveroVersion returns the version of the host that a new application
// requires.
//
// The binary reads the version of the host from its own build, so a release
// writes the version that it was built against, and a constant that a person
// must raise after each tag is gone. The CLI is a module of its own, so the
// version stands in the dependencies of the build and not in its main module.
//
...
func AveroVersion() string {
	...
	for _, dep := range info.Deps {
		if dep == nil || dep.Path != HostModule {
			continue
		}
		if replaced := dep.Replace; replaced != nil {
			// A replace directive points the build at a working copy, which
			// states no version that another application can require.
			return released(replaced.Version)
		}
...
// released returns a version that names a release, or the empty string.
