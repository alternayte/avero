...
//
// A directory that holds no drel.yaml takes the default file, because the
// first slice of an application that adopts Avero must come from `avero slice`
// alone. drel refuses a file with an empty modules block, so Avero writes the
// file and the first entry in one step.
//
...
func readSliceConfig(dir string) (string, error) {
	...
	if err != nil {
		if _, goErr := os.Stat(filepath.Join(dir, "go.mod")); goErr != nil {
			return "", sliceFaultOf("this directory holds no go.mod",
				"Run the command in the directory of a Go application, or write one with `avero new`")
		}
		return defaultDrelConfig(dir), nil
	}
	source := string(body)
...
// defaultDrelConfig returns the drel.yaml of an application that holds none.
//
// The dialect follows DATABASE_URL, because the migrations that drel writes
// carry the types of one server. The environment of the process states it
// first, then .env.
func defaultDrelConfig(dir string) string {
	return "# drel writes the models, the repositories and the migrations of each feature.\n" +
		"#\n" +
		"# One module for each feature slice, so a slice owns its models and its\n" +
		"# migrations. Delete the directory of a slice and the feature is gone.\n" +
		"#\n" +
		"# `avero generate` runs `go tool drel generate`.\n" +
		"# `avero migrate new create_comments` runs `go tool drel migrate new`.\n" +
		"modules:\n" +
		"\noutput:\n  db: ./internal/db/drel_gen.go\n" +
		"\n# The dialect of the code generation. Write postgres when DATABASE_URL names\n" +
		"# a PostgreSQL server. Write the migrations again.\n" +
		"dialect: " + dialectOf(dir) + "\n"
}

// dialectOf returns the dialect that DATABASE_URL names.
func dialectOf(dir string) string {
	url := os.Getenv("DATABASE_URL")
	if url == "" {
		url = envValue(filepath.Join(dir, ".env"), "DATABASE_URL")
	}
...
// envValue returns the value of one name of an env file, or the empty string.
func envValue(name, key string) string {
	...
	for _, line := range strings.Split(string(body), "\n") {
		...
		if !found || strings.TrimSpace(head) != key {
			continue
		}
...
func addSliceModule(dir, source string, values sliceData) ([]string, error) {
	...
	if i < 0 && strings.HasPrefix(source, "modules:\n") {
		marker = "modules:\n"
		i = 0
	}
	if i < 0 {
