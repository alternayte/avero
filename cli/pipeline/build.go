// Package pipeline builds the assets of an application: it bundles the entry
// points with esbuild, compiles the stylesheet with Tailwind, pins a module or
// a component library, and writes the manifest that the application serves.
//
// The pipeline runs at build time only. `avero build` and `avero dev` call it.
// An application imports the assets package, which holds the manifest and the
// two handlers, and it therefore compiles no bundler. See the SDD, S11.
package pipeline
...
// tier built it.
func Build(ctx context.Context, cfg Config) (*assets.Manifest, error) {
	dir, err := filepath.Abs(cfg.Dir)
	if err != nil {
		return nil, assets.NewFault(cfg.Dir, "the application directory does not resolve",
...
func buildBundled(ctx context.Context, cfg Config) (*assets.Manifest, error) {
	outDir := filepath.Join(cfg.Dir, filepath.FromSlash(cfg.out()))
	if err := os.MkdirAll(outDir, 0o755); err != nil {
		return nil, assets.NewFault(cfg.out(), "the output directory does not open",
			"Give the process the right to write the output directory")
	}
	m := assets.NewManifest(cfg.Base)
	var faults []*assets.Fault
	if cfg.Tailwind != nil {
	...
	if len(faults) > 0 {
		return nil, &assets.Faults{Faults: faults}
	}
...
// under their paths.
func record(m *assets.Manifest, outDir, name string, body []byte) error {
...
func bundle(cfg Config, outDir, entry string) ([]output, []*assets.Fault) {
	abs := filepath.Join(cfg.Dir, filepath.FromSlash(cfg.source()), filepath.FromSlash(entry))
	if _, err := os.Stat(abs); err != nil {
		return nil, []*assets.Fault{{
...
func messageFaults(dir string, msgs []api.Message) []*assets.Fault {
	out := make([]*assets.Fault, 0, len(msgs))
	for _, msg := range msgs {
		f := &assets.Fault{Message: msg.Text, Repair: repairOf(msg)}
		if msg.Location != nil {
...
func writeAsset(outDir, name string, body []byte) (assets.Entry, error) {
	hash := assets.Hash(body)
	file := assets.HashedName(name, hash)
	full := filepath.Join(outDir, filepath.FromSlash(file))
	if err := os.MkdirAll(filepath.Dir(full), 0o755); err != nil {
		return assets.Entry{}, assets.NewFault(file, "the output directory does not open",
			"Give the process the right to write the output directory")
	}
	if err := os.WriteFile(full, body, 0o644); err != nil {
		return assets.Entry{}, assets.NewFault(file, "the asset does not write",
			"Give the process the right to write the output directory")
	}
	return assets.Entry{
		Source:    name,
		...
		Hash:      hash,
		Size:      int64(len(body)),
		Integrity: assets.Integrity(body),
	}, nil
}

// writeManifest writes the manifest into the output directory.
func writeManifest(outDir string, m *assets.Manifest) error {
	body, err := json.Marshal(m)
	if err != nil {
		return assets.NewFault(assets.ManifestName, "the manifest does not encode",
			"Report the fault, because a manifest always encodes")
	}
	if err := os.WriteFile(filepath.Join(outDir, assets.ManifestName), append(body, '\n'), 0o644); err != nil {
		return assets.NewFault(assets.ManifestName, "the manifest does not write",
			"Give the process the right to write the output directory")
	}
	return nil
}
...
func buildExternal(ctx context.Context, cfg Config) (*assets.Manifest, error) {
	if len(cfg.Command) == 0 {
		return nil, assets.NewFault("", "the external bundler names no command",
	...
	if err != nil {
		return nil, assets.NewFault("", fmt.Sprintf("the bundler command failed: %v\n%s", err, strings.TrimSpace(string(out))),
	...
	if cfg.NoManifest {
		...
		return assets.NewManifest(cfg.Base), nil
	}
	name := filepath.Join(cfg.Dir, filepath.FromSlash(cfg.out()), assets.ManifestName)
	body, err := os.ReadFile(name)
	if err != nil {
		return nil, assets.NewFault(path.Join(cfg.out(), assets.ManifestName),
			fmt.Sprintf("the bundler command wrote no %s", assets.ManifestName),
