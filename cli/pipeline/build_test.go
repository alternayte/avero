...
func config(dir string) pipeline.Config {
...
func TestTierZeroBundlesWithNoNodeModules(t *testing.T) {
	dir := project(t)
	m, err := pipeline.Build(context.Background(), config(dir))
	if err != nil {
...
func TestAChangedCSSFileProducesADifferentHash(t *testing.T) {
	dir := project(t)
	first, err := pipeline.Build(context.Background(), config(dir))
	if err != nil {
		t.Fatalf("Build returned %v, want nil", err)
	}
	write(t, dir, "assets/css/base.css", "body { margin: 8px; }\n")
	second, err := pipeline.Build(context.Background(), config(dir))
	if err != nil {
		t.Fatalf("Build returned %v, want nil", err)
	}
...
func TestTierZeroRefusesABareSpecifier(t *testing.T) {
	...
	_, err := pipeline.Build(context.Background(), config(dir))
	if err == nil {
...
func TestTierExternalRunsTheCommand(t *testing.T) {
	...
	built, err := pipeline.Build(context.Background(), cfg)
	if err != nil {
		t.Fatalf("Build returned %v, want nil", err)
	}
...
func TestTierExternalNamesTheRepairWhenTheCommandWritesNoManifest(t *testing.T) {
	...
	_, err := pipeline.Build(context.Background(), cfg)
	if err == nil {
		t.Fatal("Build returned nil, want a fault")
	}
