...
func TestAScaffoldedApplicationBuildsWithNoNodeAndServesTheAssets(t *testing.T) {
	...
	if err := pipeline.PinJS(ctx, pipeline.PinConfig{Dir: dir}, "nanostores", nanoStores); err != nil {
	...
	t.Setenv("PATH", "")
	m, err := pipeline.Build(ctx, pipeline.Config{
		Dir:      dir,
		Entries:  []string{"js/app.js"},
		Minify:   true,
		Tailwind: &pipeline.Tailwind{Dir: dir, Input: "assets/css/app.css", Minify: true},
	})
	if err != nil {
