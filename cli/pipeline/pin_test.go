...
func TestPinWritesTheVendorFileAndTheLock(t *testing.T) {
	...
	if err := pipeline.PinJS(context.Background(), pipeline.PinConfig{Dir: dir, Fetch: f}, "nanostores", nanoURL); err != nil {
		t.Fatalf("PinJS returned %v, want nil", err)
	}
	body, err := os.ReadFile(filepath.Join(dir, filepath.FromSlash(pipeline.VendorDir), "nanostores.js"))
	if err != nil {
		t.Fatalf("the vendor file is absent: %v", err)
	}
	sum := sha256.Sum256(body)

	lock, err := pipeline.LoadLock(dir)
	if err != nil {
...
func TestPinIsIdempotent(t *testing.T) {
	dir := t.TempDir()
	f := &fetcher{bodies: map[string]string{nanoURL: "export const atom = () => 1;\n"}}
	cfg := pipeline.PinConfig{Dir: dir, Fetch: f}

	for i := range 3 {
		if err := pipeline.PinJS(context.Background(), cfg, "nanostores", nanoURL); err != nil {
...
func TestPinFailsOnAHashMismatch(t *testing.T) {
	...
	if err := os.Remove(filepath.Join(dir, filepath.FromSlash(pipeline.VendorDir), "nanostores.js")); err != nil {
		t.Fatalf("Remove returned %v", err)
	}
	f.bodies[nanoURL] = "export const atom = () => 2;\n"
	err := pipeline.PinJS(context.Background(), cfg, "nanostores", nanoURL)
	if err == nil {
		t.Fatal("PinJS returned nil, want a fault")
	}
	if !strings.Contains(err.Error(), "nanostores") || !strings.Contains(err.Error(), pipeline.LockName) {
...
func TestPinRepairsAnAbsentVendorFile(t *testing.T) {
	...
	file := filepath.Join(dir, filepath.FromSlash(pipeline.VendorDir), "nanostores.js")
...
func TestPinWithNoURLReadsTheLock(t *testing.T) {
	...
	if err := pipeline.PinJS(context.Background(), cfg, "nanostores", ""); err != nil {
		t.Fatalf("PinJS returned %v, want nil", err)
	}
	if err := pipeline.PinJS(context.Background(), cfg, "absent", ""); err == nil {
...
func TestTailwindDownloadsTheBinaryOneTime(t *testing.T) {
	...
	dir := t.TempDir()
	tw := &pipeline.Tailwind{Version: "4.1.0", Dir: dir}
...
func TestTailwindCompilesTheStylesheet(t *testing.T) {
	...
	tw := &pipeline.Tailwind{Version: "4.1.0", Dir: dir, Input: "assets/css/app.css"}
...
func TestTheBuildTakesTheTailwindOutput(t *testing.T) {
	...
	dir := project(t)
	tw := &pipeline.Tailwind{Version: "4.1.0", Dir: dir, Input: "assets/css/app.css", Asset: "css/app.css"}
	tw.Fetch = &fetcher{bodies: map[string]string{tw.URL(): "#!/bin/sh\nwhile [ $# -gt 0 ]; do if [ \"$1\" = \"-o\" ]; then printf 'h1{color:blue}' > \"$2\"; fi; shift; done\n"}}

	cfg := pipeline.Config{Dir: dir, Entries: []string{"js/app.js"}, Tailwind: tw}
	m, err := pipeline.Build(context.Background(), cfg)
	if err != nil {
...
func TestResolveReadsTheModuleOfAPackage(t *testing.T) {
	...
	url, err := pipeline.Resolve(context.Background(), f, "zustand")
	if err != nil {
...
func TestResolveTakesTheAnswerWhenItHoldsTheModule(t *testing.T) {
	...
	url, err := pipeline.Resolve(context.Background(), f, "tiny")
	if err != nil {
		t.Fatalf("Resolve returned %v, want nil", err)
	}
...
func TestTheLockRecordsAPackage(t *testing.T) {
	...
	lock.SetPackage("basecoat", pipeline.Pin{
	...
	again, err := pipeline.LoadLock(dir)
	if err != nil {
		t.Fatalf("LoadLock returned %v, want nil", err)
	}
...
func TestPinResolvesAPackageWithNoAddress(t *testing.T) {
	...
	if err := pipeline.PinJS(context.Background(), pipeline.PinConfig{Dir: dir, Fetch: f}, "zustand", ""); err != nil {
		t.Fatalf("PinJS returned %v, want nil", err)
	}
	if _, err := os.Stat(filepath.Join(dir, filepath.FromSlash(pipeline.VendorDir), "zustand.js")); err != nil {
		t.Fatalf("the vendor file is absent: %v", err)
	}
	lock, err := pipeline.LoadLock(dir)
	if err != nil {
...
func TestPinNamesTheRepairForAPackageThatDoesNotResolve(t *testing.T) {
	f := &fetcher{bodies: map[string]string{}}
	err := pipeline.PinJS(context.Background(), pipeline.PinConfig{Dir: t.TempDir(), Fetch: f}, "absent-package", "")
	if err == nil {
		t.Fatal("PinJS returned nil, want a fault")
	}
