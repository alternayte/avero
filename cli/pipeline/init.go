...
func Init(dir, name string) (bool, error) {
	...
	if _, err := os.Stat(file); err == nil {
		...
		return false, assets.NewFault(PackageName, "the package file does not open",
	...
	if err != nil {
		return false, assets.NewFault(PackageName, "the package file does not encode",
			"Report the fault, because the document always encodes")
	}
	if err := os.WriteFile(file, append(body, '\n'), 0o644); err != nil {
		return false, assets.NewFault(PackageName, "the package file does not write",
