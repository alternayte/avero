...
func PinPackage(ctx context.Context, cfg PinConfig, name, url string) error {
	...
	if url == "" {
		return assets.NewFault(LockName, fmt.Sprintf("the package %q states no address", name),
	...
	if err != nil {
		return assets.NewFault(LockName, fmt.Sprintf("the package %q does not download from %s: %v", name, url, err),
			"Prove the address in a browser. Run the command again.")
	}
	sum := sha256.Sum256(body)
	hash := hex.EncodeToString(sum[:])
	if locked && pin.URL == url && pin.SHA256 != hash {
		return assets.NewFault(LockName,
	...
	if len(files) == 0 {
		return assets.NewFault(LockName, fmt.Sprintf("the package %q holds no %s directory", name, distPrefix),
...
func removeStaleFiles(root string, old, current []string) error {
	...
	for _, name := range old {
		...
		if err := os.Remove(file); err != nil && !os.IsNotExist(err) {
			return assets.NewFault(path.Join(PackageDir, name), "the file of the older release does not delete",
...
func extract(body []byte, root, name string) ([]string, error) {
	...
	if err != nil {
		return nil, assets.NewFault(LockName, fmt.Sprintf("the package %q does not open as a gzip file", name),
	...
	for {
		...
		if err != nil {
			return nil, assets.NewFault(LockName, fmt.Sprintf("the package %q does not read as a tarball", name),
				"Prove that the address names a tarball of npm. Run the command again.")
		}
		...
		if len(files)+1 > maxPackageFiles {
			return nil, assets.NewFault(LockName,
...
func writeMember(root, member string, in io.Reader, size int64) error {
	...
	if err := os.MkdirAll(filepath.Dir(file), 0o755); err != nil {
		return assets.NewFault(path.Join(PackageDir, member), "the vendor directory does not open",
			"Give the process the right to write the assets directory")
	}
	out, err := os.Create(file)
	if err != nil {
		return assets.NewFault(path.Join(PackageDir, member), "the file does not write",
			"Give the process the right to write the assets directory")
	}
	defer func() { _ = out.Close() }()
	if _, err := io.CopyN(out, in, size); err != nil {
		return assets.NewFault(path.Join(PackageDir, member), "the file ends before its declared size",
