...
func LoadLock(dir string) (*Lock, error) {
	...
	if err != nil {
		return nil, assets.NewFault(LockName, "the lock does not open",
			"Give the process the right to read the lock")
	}
	var doc lockJSON
	if err := json.Unmarshal(body, &doc); err != nil {
		return nil, assets.NewFault(LockName, "the lock does not parse",
...
func (l *Lock) Save() error {
	...
	if err != nil {
		return assets.NewFault(LockName, "the lock does not encode",
			"Report the fault, because a lock always encodes")
	}
	if err := os.WriteFile(l.path, append(body, '\n'), 0o644); err != nil {
		return assets.NewFault(LockName, "the lock does not write",
...
func PinJS(ctx context.Context, cfg PinConfig, name, url string) error {
	...
	if err != nil {
		return assets.NewFault(LockName, fmt.Sprintf("the module %q does not download from %s: %v", name, url, err),
	...
	if err := os.MkdirAll(filepath.Dir(file), 0o755); err != nil {
		return assets.NewFault(path.Join(VendorDir, name+".js"), "the vendor directory does not open",
			"Give the process the right to write the assets directory")
	}
	if err := os.WriteFile(file, body, 0o644); err != nil {
		return assets.NewFault(path.Join(VendorDir, name+".js"), "the module does not write",
			"Give the process the right to write the assets directory")
	}
