...
func (t *Tailwind) Ensure(ctx context.Context) (string, error) {
	...
	if err != nil {
		return "", assets.NewFault(path.Join(BinDir, t.Name()),
	...
	if err := os.MkdirAll(filepath.Dir(file), 0o755); err != nil {
		return "", assets.NewFault(BinDir, "the binary directory does not open",
			"Give the process the right to write the application directory")
	}
	if err := os.WriteFile(file, body, 0o755); err != nil {
		return "", assets.NewFault(path.Join(BinDir, t.Name()), "the Tailwind binary does not write",
			"Give the process the right to write the application directory")
	}
...
func (t *Tailwind) Compile(ctx context.Context) ([]byte, error) {
	...
	if err != nil {
		return nil, assets.NewFault(t.Input, "the temporary stylesheet does not open",
	...
	if body, err := cmd.CombinedOutput(); err != nil {
		return nil, assets.NewFault(t.Input,
	...
	if err != nil {
		return nil, assets.NewFault(t.Input, "Tailwind wrote no stylesheet",
