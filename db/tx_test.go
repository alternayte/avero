...
// errBoom is the error that a handler of this suite returns.
var errBoom = errors.New("the handler failed")

// captureLogger returns a logger that writes into a buffer, and a reader of
// that buffer.
...
// serve builds the handler of the router and runs one request against it.
...
func write(c *router.Ctx, name string) error {
	_, err := db.MustTx(c).Exec(c.Context(), `INSERT INTO things (name) VALUES (?)`, name)
	return err
}
...
func TestAPanicRollsBackAndReturns500(t *testing.T) {
	...
	r.Use(router.Recover(log), db.Transaction(e))
	r.Post("/things", func(c *router.Ctx) (router.Response, error) {
		if err := write(c, "lost"); err != nil {
			return nil, err
		}
...
func TestTheHandlerReadsTheTransactionFromTheContext(t *testing.T) {
	...
	r.Get("/x", func(c *router.Ctx) (router.Response, error) {
		if _, ok := db.Tx(c); !ok {
			t.Fatal("db.Tx found no transaction inside a transaction")
		}
...
func TestTxIsNilWithNoTransactionMiddleware(t *testing.T) {
	...
	r.Get("/x", func(c *router.Ctx) (router.Response, error) {
		if _, ok := db.Tx(c); ok {
			t.Fatal("db.Tx found a transaction with no middleware")
		}
		return router.NoContent(), nil
	})
	serve(t, r, http.MethodGet, "/x")
